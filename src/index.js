...
const nodes = [
    ...
    },
    {
        ...
        description: `Only 8.6% of cases reported to police result in a guilty conviction, according to <a href="http://www.mrc.ac.za/sites/default/files/files/2017-10-30/RAPSSAreport.pdf">"Rape Justice in South Africa"</a>. Of those convictions, 72.6% of the rapists would ever see jail time. That means that of the ${ number_format(total_rapes * 9) } rapes that we started with, only ${ number_format(total_rapes * 0.086 * 0.726) } (${ number_format((total_rapes * 0.086 * 0.726) / (total_rapes * 9) * 100) }%) result in prison for the perpetrator.`
    }
]
